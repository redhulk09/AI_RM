...
def _db_transaction(result: dict[str, Any]) -> Transaction:
    ...
    return Transaction(
        transaction_id=result["transaction_id"], amount=result["amount"], customer_id=result["customer_id"],
        timestamp=timestamp, device_id=result["device_id"], country=result["country"], payment_method=result["payment_method"],
        account_age_days=result["account_age_days"], transactions_last_10m=result["transactions_last_10m"],
        transactions_last_1h=result["transactions_last_1h"], failed_payments=result["failed_payments"],
        previous_avg_amount=result["previous_avg_amount"], amount_deviation=result["amount_deviation"],
        is_new_device=int(result["is_new_device"]), is_new_location=int(result["is_new_location"]),
        distance_from_previous_location=result["distance_from_previous_location"], device_transaction_count=result["device_transaction_count"],
        ip_transaction_count=result["ip_transaction_count"], hour=result["hour"], customer_transaction_count=result["customer_transaction_count"],
        currency=result.get("currency", "INR"),
    )


def save_prediction(db: Session, result: dict[str, Any]) -> None:
    existing = db.scalar(select(Transaction).where(Transaction.transaction_id == result["transaction_id"]))
    if existing:
        transaction = existing
    else:
        transaction = _db_transaction(result)
        db.add(transaction)
    db.add(
        Prediction(
            transaction_id=result["transaction_id"],
            risk_score=result["risk_score"], risk_level=result["risk_level"],
            fraud_probability=result["fraud_probability"], reasons=json.dumps(result["reasons"]),
...
def serialize_prediction(result: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "recommended_action": result["recommended_action"],
        "transaction": {k: v for k, v in result.items() if k not in {"reasons", "top_features", "risk_score", "risk_level", "fraud_probability", "recommended_action", "threshold_used", "flagged"}},
    }
...
def predict_and_save(db: Session, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    results = predict_rows([payload], threshold=float(payload.get("threshold", 0.70)))
    result = results[0]
...
def recent_transactions(db: Session, limit: int = 20) -> list[dict[str, Any]]:
    ...
    for transaction, prediction in rows:
        ...
        output.append({
            ...
            "country": transaction.country, "payment_method": transaction.payment_method,
            "risk_score": prediction.risk_score, "risk_level": prediction.risk_level,
            "fraud_probability": prediction.fraud_probability, "reasons": json.loads(prediction.reasons),
...
def transaction_detail(db: Session, transaction_id: str) -> dict[str, Any] | None:
    ...
    return {
        ...
        "amount_deviation": transaction.amount_deviation, "is_new_device": bool(transaction.is_new_device),
        "is_new_location": bool(transaction.is_new_location), "distance_from_previous_location": transaction.distance_from_previous_location,
        "device_transaction_count": transaction.device_transaction_count, "ip_transaction_count": transaction.ip_transaction_count,
        "customer_transaction_count": transaction.customer_transaction_count, "risk_score": prediction.risk_score,
        "risk_level": prediction.risk_level, "fraud_probability": prediction.fraud_probability,
