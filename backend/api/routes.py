...
from ..schemas import BatchResult, PredictionOut, TransactionInput
...
def batch_predict(file: UploadFile = File(...), threshold: float = Query(0.70, ge=0.01, le=0.99), db: Session = Depends(get_db)):
    ...
    try:
        ...
        for result in predictions:
            save_prediction(db, result, commit=False)
        db.commit()
...
def _csv_row_to_transaction(row: dict[str, str], index: int) -> dict[str, Any]:
...
def _prediction_response(item: dict[str, Any]) -> dict[str, Any]:
    hidden = {"risk_score", "risk_level", "fraud_probability", "reasons", "top_features", "recommended_action", "threshold_used", "flagged"}
    return {"transaction_id": item["transaction_id"], "risk_score": item["risk_score"], "risk_level": item["risk_level"], "fraud_probability": item["fraud_probability"], "reasons": item["reasons"], "top_features": item["top_features"], "recommended_action": item["recommended_action"], "transaction": {k: v for k, v in item.items() if k not in hidden}}
...
def demo_seed(db: Session = Depends(get_db)):
    try:
        from scripts.seed_demo import seed_demo_rows
